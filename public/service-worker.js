const CACHE_VERSION = 'v1.0.1'; // Increment this with each app update that needs cache busting
...
self.addEventListener('fetch', (event) => {
  ...
  // Strategy: Cache first, then network for other assets (JS, CSS, images, API calls etc.)
  // These are typically versioned by hash or have their own caching headers from the server.
  event.respondWith(
      ...
      .then((cachedResponse) => {
        ...
        return fetch(event.request).then((networkResponse) => {
          if (networkResponse && networkResponse.ok) {
            // For non-HTML, non-navigation requests, we might still want to cache them if they are successful GETs
            // For example, API responses for offline use, or images/fonts not in the initial precache.
            if (event.request.method === 'GET' && !url.pathname.startsWith('/api/auth')) { // Example: don't cache auth API calls
          ...
          console.warn('[Service Worker] Fetch failed, returning error response:', error);
